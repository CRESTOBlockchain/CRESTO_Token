...
contract CrestoCrowdsale{
    ...
    //CrestoToken.sol address (must be deployed first)
    CrestoToken _tokenReward;
    ...
    function CrestoCrowdsale(address addressOfTokenUsedAsReward) public {
        _owner = msg.sender;
        _tokenReward = CrestoToken(addressOfTokenUsedAsReward);
        _start = now;
        _end = now + 31 days;
    }
